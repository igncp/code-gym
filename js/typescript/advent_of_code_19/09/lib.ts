...
interface IntcodeProgramState {
  program: IntcodeProgram;
  position: ExecutionPosition;
  relativeBase: ExecutionPosition;
}
...
const getInstructionConfig: GetInstructionConfig = num => {
  ...
  if (numStr.length > 2) {
    ...
    for (let numPos = paramsModeStart; numPos >= 0; numPos -= 1) {
      ...
      if (numStr[numPos] === "2") {
        paramsModes[paramPos] = ParameterMode.Relative;
      } else {
  ...
  if (
    opcodeNum === Opcode.Input ||
    opcodeNum === Opcode.Output ||
    opcodeNum === Opcode.AdjustRelative
  ) {
    positions = 2;
...
const runProgramOperation: RunProgramOperation = (
  { program, position, relativeBase },
  opts = {}
) => {
  const newProgram = program.slice(0);
  let nextRelativeBase = relativeBase || 0;
  ...
  const getInput = (posNum: number): number => {
    ...
    if (paramsModes[posNum] === ParameterMode.Relative) {
      return newProgram[inputPosition + nextRelativeBase];
    }
  ...
  const setOutput = (posNum: number, val: number) => {
    const outputPosition =
      newProgram[position + posNum] +
      (paramsModes[posNum] === ParameterMode.Relative ? nextRelativeBase : 0);
  ...
  if (opcode === Opcode.Addition) {
    ...
    const result = getInput(1) === getInput(2);
    setOutput(3, result ? 1 : 0);
  } else if (opcode === Opcode.AdjustRelative) {
    const value = getInput(1);

    nextRelativeBase += value;
...
const getVerbAndNounForResult: GetVerbAndNounForResult = (program, result) => {
  for (let verb = 0; verb <= 99; verb += 1) {
    for (let noun = 0; noun <= 99; noun += 1) {
      ...
      const { program: programResult } = runProgramTillEnd({
        program: programCopy,
        position: 0,
        relativeBase: 0
      });
    ...
    runPhaseSettings([5, 6, 7, 8, 9], phaseSettings => {
      ...
      while (counter < 100 * 100) {
        ...
        const {
          ...
          {
            program: programs[ampIdx],
            position: positions[ampIdx],
            relativeBase: 0
          },
          {
            getShouldHalt: () => {
